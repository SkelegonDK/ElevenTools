...
def cleanup_old_sessions(max_age_hours: int = 24) -> int:
    ...
    logger.info(
        f"Starting cleanup of session directories older than {max_age_hours} hours"
    )
            ...
            if os.path.isdir(session_dir):
                ...
                try:
                    ...
                    if dir_age > max_age_seconds:
                        logger.info(
                            f"Removing old session directory: {entry} (age: {dir_age_hours:.1f} hours)"
                        )
                        shutil.rmtree(session_dir)
                        removed_count += 1
                    else:
                        logger.debug(
                            f"Preserving session directory: {entry} (age: {dir_age_hours:.1f} hours)"
                        )
    ...
    if removed_count > 0:
        logger.info(
            f"Cleanup completed: removed {removed_count} old session directory(ies)"
        )
    else:
