...
def get_session_state_size(session_state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "largest_items": [(key, size, round(size / (1024 * 1024), 2)) for key, size in largest_items]
    }


def log_session_state_memory(level: str = "INFO", session_state: Optional[Dict[str, Any]] = None) -> None:
    ...
    try:
        ...
        if level.upper() == "DEBUG":
            ...
            if size_info['total_size_mb'] > 50:  # Warn if over 50MB
                logger.warning(f"High memory usage detected! Largest items: {size_info['largest_items']}")
        else:
...
def monitor_memory_usage(
    ...
    if log_before:
        logger.debug(f"[{operation_name}] Memory before: {size_info['total_size_mb']} MB")
    
    if log_after:
        logger.debug(f"[{operation_name}] Memory after: {size_info['total_size_mb']} MB")
