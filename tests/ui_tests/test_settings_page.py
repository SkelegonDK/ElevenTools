...
class TestSettingsPage:
    ...
    def test_page_loads(self, configured_page: Page, streamlit_server: str):
        ...
        title_or_content = (
            ...
            if configured_page.get_by_text(re.compile(r"settings", re.IGNORECASE)).count() > 0
    ...
    def test_page_title_display(self, configured_page: Page, streamlit_server: str):
        ...
        settings_title = configured_page.get_by_text(re.compile(r"settings", re.IGNORECASE))
    ...
    def test_api_keys_section_present(self, configured_page: Page, streamlit_server: str):
        ...
        api_keys_header = configured_page.get_by_text(re.compile(r"api.*key.*management", re.IGNORECASE))
    ...
    def test_default_model_configuration_section_present(self, configured_page: Page, streamlit_server: str):
        ...
        model_config_header = configured_page.get_by_text(re.compile(r"default.*model.*configuration", re.IGNORECASE))
        expect(model_config_header.first).to_be_visible()

    def test_default_translation_model_section_present(self, configured_page: Page, streamlit_server: str):
        ...
        translation_model_header = configured_page.get_by_text(re.compile(r"default.*translation.*model", re.IGNORECASE))
    ...
    def test_default_enhancement_model_section_present(self, configured_page: Page, streamlit_server: str):
        ...
        enhancement_model_header = configured_page.get_by_text(re.compile(r"default.*script.*enhancement.*model", re.IGNORECASE))
        # May not be visible if API key is missing, so check conditionally
    ...
    def test_free_model_filter_present(self, configured_page: Page, streamlit_server: str):
        ...
        free_filter = configured_page.get_by_text(re.compile(r"show.*only.*free.*models", re.IGNORECASE))
        # May not be visible if API key is missing, so check conditionally
        if free_filter.count() > 0:
            expect(free_filter.first).to_be_visible()

    def test_model_selectbox_present(self, configured_page: Page, streamlit_server: str):
        ...
        assert selectboxes.count() >= 0  # At least zero (might not be visible if no API key)
    ...
    def test_status_indicators_present(self, configured_page: Page, streamlit_server: str):
        ...
        status_elements = (
            ...
            else configured_page.get_by_text(re.compile(r"configured|not.*found|key|enabled|disabled", re.IGNORECASE))
        )
