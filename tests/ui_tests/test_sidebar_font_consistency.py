...
class TestSidebarFontConsistency:
    ...
    def test_sidebar_font_before_button_press(
    ...
    def test_sidebar_font_after_button_press(
        self, configured_page: Page, streamlit_server: str
    ):
        ...
        expander_button = configured_page.locator(
        ...
        if expander_button.count() > 0:
            ...
            action_buttons = configured_page.locator(
                'button:not([data-testid="stBaseButton-header"])'
            )
        ...
        if initial_font and font_after:
            font_changed = (
                initial_font["fontFamily"] != font_after["fontFamily"]
                or initial_font["fontWeight"] != font_after["fontWeight"]
            )

            if font_changed:
                print("\n[WARNING] Font changed after button press!")
                print(
                    f"  Before: {initial_font['fontFamily']} / {initial_font['fontWeight']}"
                )
                print(
                    f"  After: {font_after['fontFamily']} / {font_after['fontWeight']}"
                )
            else:
                print("\n[OK] Font remained consistent")

        assert (
            font_after is not None
        ), "Sidebar font should be accessible after button press"
    ...
    def test_sidebar_font_multiple_interactions(
        self, configured_page: Page, streamlit_server: str
    ):
        ...
        for i in range(3):
            ...
            expander_buttons = configured_page.locator(
                '[data-testid="stExpander"] button'
            )
    ...
    def test_sidebar_font_across_pages(
        self, configured_page: Page, streamlit_server: str
    ):
        ...
        bulk_link = configured_page.get_by_role(
            "link", name="Bulk Generation", exact=False
        )
        if bulk_link.count() > 0:
            ...
            if main_page_font and bulk_page_font:
                assert (
                    main_page_font["fontFamily"] == bulk_page_font["fontFamily"]
                ), f"Font family differs between pages: main={main_page_font['fontFamily']}, bulk={bulk_page_font['fontFamily']}"
                assert (
                    main_page_font["fontWeight"] == bulk_page_font["fontWeight"]
