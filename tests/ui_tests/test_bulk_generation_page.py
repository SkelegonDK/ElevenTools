...
class TestBulkGenerationPage:
    ...
    def test_generate_button_present(self, configured_page: Page, streamlit_server: str):
        ...
        generate_button = buttons.filter(has_text=re.compile(r'Generate.*Bulk|Bulk.*Audio', re.IGNORECASE))
        
        if generate_button.count() == 0:
            generate_button = configured_page.get_by_role("button", name=re.compile(r'Generate.*Bulk|Bulk.*Audio', re.IGNORECASE))
    ...
    def test_info_message_when_no_file(self, configured_page: Page, streamlit_server: str):
        ...
        info_text = configured_page.get_by_text(re.compile(r"upload.*csv|csv.*file", re.IGNORECASE))
